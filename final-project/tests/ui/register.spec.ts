...
test.describe("Сторінка реєстрації", () => {
  test("Всі UI елементі наявні на сторінці", async ({ page }) => {
    ...
    await expect(register.form.email).toBeVisible();
    await expect(register.form.password).toBeVisible();
    await expect(register.form.confirmPassword).toBeVisible();
    await expect(register.form.groupSelect).toBeVisible();
    await expect(register.form.submitButton).toBeVisible();
  });
